...
class ConsentService:
    ...
    async def check_consent(
        ...
        if db_consent and getattr(db_consent, 'granted', False):
            consent_scopes = getattr(db_consent, 'scopes', []) or []
            granted_scopes = set(consent_scopes)
            return {
                scope.value: scope.value in granted_scopes
                for scope in required_scopes
            }
    ...
    async def get_user_consents(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        db_consent = self.db_service.get_user_consent(user_id)
        if db_consent:
            expires_at = getattr(db_consent, 'expires_at', None)
            return [{
                "user_id": getattr(db_consent, 'user_id', user_id),
                "scopes": getattr(db_consent, 'scopes', []),
                "granted": getattr(db_consent, 'granted', False),
                "expires_at": expires_at.isoformat() if expires_at else None,
                "consent_token": getattr(db_consent, 'consent_token', '')
            }]
